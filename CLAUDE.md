...
- **PENDIENTE:** Feed anti-pseudociencia (lógica de filtrado en posts)
...
### Features implementadas en este sprint

#### 1. Avatar Glow por Rareza de Badge Featured
**Archivo:** `mobile/components/profile/ProfileHeader.tsx`

La foto de perfil tiene un borde luminoso (glow) cuyo color y intensidad dependen de la rareza del badge marcado como "featured" (destacado) en el perfil del usuario.

```
COMMON    → Borde gris #A0A0A0, sin sombra (shadowRadius: 0)
RARE      → Borde azul #3B82F6, borderWidth: 3, shadowRadius: 8, shadowOpacity: 0.4
EPIC      → Borde morado #8B5CF6, borderWidth: 3, shadowRadius: 12, shadowOpacity: 0.5
LEGENDARY → Borde dorado #F59E0B, borderWidth: 4, shadowRadius: 18, shadowOpacity: 0.7
```

Implementación: config map `AVATAR_GLOW` indexado por `BadgeRarity`. El `<View>` wrapper del avatar aplica `shadowColor`, `shadowOffset: {0,0}`, `shadowOpacity`, `shadowRadius`, `elevation` (Android). El `<Image>` usa `borderWidth` y `borderColor` del config.

Si el usuario no tiene badge featured → default a COMMON (sin glow).

#### 2. Fix Crash en Charts: `TypeError: data.map is not a function`

**Error original:** `StepsChart` (y otros charts internos de `ChartsTab`) crasheaban con `TypeError: data.map is not a function (it is undefined)` cuando la API retornaba un objeto de error `{ error: "mensaje" }` en vez del shape esperado `{ type: "steps", data: [...] }`.

**Causa raíz:** Los hooks de charts (`useNutritionHistory`, `useStepsHistory`, etc.) no manejaban errores de API — si la request fallaba, `chartData.data` quedaba `undefined`. Los componentes de chart asumían que siempre recibirían un array y llamaban `.map()` directamente.

**Fix aplicado en `ChartsTab.tsx`:**
- Función `renderChart` usa `Array.isArray(d) ? d : []` para todos los charts que esperan arrays antes de pasarlos al componente
- Guards específicos por tipo de chart:
  - `HeatmapChart`: guard `typeof data !== "object" || data === null`
  - `StrengthChart`: guard `!Array.isArray(data)` → retorna `null` si no es array
  - `MacrosChart`: guard `!data?.protein || !data?.carbs || !data?.fat` → retorna null si faltan macros

**Patrón defensivo a seguir:** Todo componente que consuma datos de API debe tener guards antes de operaciones como `.map()`, `.filter()`, `.reduce()`.

#### 3. Nombre y Username en Sign-up

**Archivos:** `mobile/app/(auth)/sign-up.tsx`, `backend/src/routes/users.ts`, `backend/src/routes/webhooks.ts`, `mobile/lib/api.ts`

**Problema anterior:** El sign-up solo guardaba el email del usuario. El nombre y username venían de Clerk (firstName, lastName del OAuth) o eran un string genérico. No había username único ni campo de nombre personalizable.

**Solución implementada:**
- Sign-up tiene 2 nuevos inputs: "Nombre" (no único, display name) y "Username" (único, identificador público)
- Username se auto-sanitiza en tiempo real: convierte a minúsculas, elimina caracteres no permitidos (solo `a-z`, `0-9`, `.`, `_`)
- Validación de disponibilidad con debounce 500ms: llama `GET /api/users/check-username/:username` mientras el usuario escribe
- Feedback visual inmediato bajo el input:
  - Gris: "Verificando..." (mientras espera respuesta API)
  - Verde: "✓ Disponible"
  - Rojo: mensaje de error (ej: "Este username ya está en uso", "Solo letras minúsculas, números, puntos y guiones bajos")
- Validación de nombre: mínimo 2 caracteres, no puede estar vacío
- Botón "Crear cuenta" deshabilitado si username no disponible o hay errores

#### 4. Flujo Optimizado con Clerk `unsafeMetadata` (decisión arquitectónica clave)

**Problema que resuelve:** El patrón anterior era:
1. Clerk crea el usuario → dispara webhook → backend crea User en DB (sin nombre/username del form)
2. La app mobile hacía `setTimeout(1500ms)` y luego llamaba `PUT /api/users/me` para actualizar nombre/username
3. Esto era frágil: race condition si el webhook tardaba, doble write, código con `setTimeout` que huele mal

**Solución:** Clerk expone `unsafeMetadata` — datos arbitrarios que se guardan en Clerk junto al usuario y se incluyen en todos los eventos de webhook como `data.unsafe_metadata`.

**Flujo nuevo (atómico):**
1. `signUp.create()` en mobile envía `{ email, password, unsafeMetadata: { displayName, username } }`
2. Clerk procesa el sign-up → dispara evento `user.created` al webhook
3. Webhook `user.created` lee `data.unsafe_metadata.displayName` y `data.unsafe_metadata.username`
4. Crea el usuario en DB con nombre y username correctos en una sola operación
5. Mobile solo verifica el código de email (`signUp.attemptEmailAddressVerification`) — ya no actualiza el perfil

**Resultado:** Sin race conditions, sin setTimeout, sin double writes. Un solo INSERT a la DB. Escalable a millones de usuarios.

**Webhook `user.updated`:** Se modificó para que NO sobreescriba `name` del usuario cuando Clerk actualiza (ej: si el usuario cambia su avatar en Clerk). Solo sincroniza `email` y `avatarUrl`. El nombre es "owned" por la app, no por Clerk.

#### 5. Edición de Nombre y Username en Perfil

**Archivo:** `mobile/components/profile/EditProfileSection.tsx`

- Nuevo campo "Username" visible en el formulario de edición de perfil
- Misma lógica de validación debounced que sign-up (código reutilizable)
- Si el usuario intenta guardar un username ya tomado:
  - Backend retorna HTTP 409 con `{ error: "Este username ya está en uso" }`
  - Mobile muestra `Alert.alert("Error", err?.response?.data?.error)`
- Botón "Guardar" deshabilitado mientras `usernameError !== null` o mientras el username está siendo verificado
- El campo muestra el username actual como valor inicial (cargado desde el perfil)
- Al guardar exitosamente, el estado local se actualiza para reflejar el nuevo username/nombre

#### 6. Endpoint de Verificación de Username

**Archivo:** `backend/src/routes/users.ts`
**Ruta:** `GET /api/users/check-username/:username`

Lógica:
1. Recibe username como param de URL (encodeURIComponent en el cliente)
2. Convierte a lowercase y trim
3. Valida longitud mínima (3 chars) → `{ available: false, reason: "..." }`
4. Valida formato regex `^[a-z0-9._]+$` → `{ available: false, reason: "..." }`
5. Busca en DB `prisma.user.findUnique({ where: { username } })`
6. Si existe y pertenece al mismo usuario que hace la request → `{ available: true }` (para edición de perfil)
7. Si existe y pertenece a otro usuario → `{ available: false, reason: "Este username ya está en uso" }`
8. Si no existe → `{ available: true }`

Requiere autenticación Clerk (usa `getAuth(req)` para obtener el clerkId del usuario actual).

---

## Archivos Modificados (pendientes de commit al cerrar esta sesión)

| Archivo | Tipo de cambio | Detalle |
|---------|---------------|---------|
| `backend/src/routes/users.ts` | Feature + Fix | Nuevo endpoint `GET /check-username/:username`. Bloque de validación de username en `PUT /me` (formato, longitud, unicidad con 409 si tomado) |
| `backend/src/routes/webhooks.ts` | Refactor | `user.created`: lee `data.unsafe_metadata` para nombre/username, genera username de fallback si falta, verifica unicidad antes de crear. `user.updated`: ya no sobreescribe `name`, solo sincroniza `email` y `avatarUrl` |
| `mobile/app/(auth)/sign-up.tsx` | Feature | Inputs de nombre y username, validación debounced, auto-sanitizado, envío de `unsafeMetadata` en `signUp.create()`, `handleVerify` simplificado sin setTimeout ni updateProfile |
| `mobile/app/(tabs)/profile.tsx` | Feature | Pasa `username: combo.username` como prop a `EditProfileSection`. También recibe ChartsTab, useNutritionHistory, useStepsHistory |
| `mobile/components/profile/EditProfileSection.tsx` | Feature | Campo username editable con misma validación debounced, manejo de error 409, botón Save deshabilitado en error |
| `mobile/lib/api.ts` | Feature | Nuevo método `userAPI.checkUsername(username: string)` que llama `GET /api/users/check-username/:encodeURIComponent(username)` |
...
#### Features pendientes urgentes
- [ ] ⚡ **Modal de comentarios** — línea ~1082 en `social.tsx` tiene TODO. Implementar thread de comentarios con `Comment.parentId` que ya existe en schema
- [ ] **Hashtags** — soporte `#fitness #gym` en posts → clickable → pantalla de búsqueda por tag
- [ ] **Menciones** — soporte `@username` en posts y comentarios → notificación al usuario mencionado
- [ ] **Guardar posts** — bookmark para leer después, sin notificar al autor

#### Feed y descubrimiento
- [ ] **Feed "Para ti"** — algoritmo que aprende qué tipo de posts le gustan al usuario
- [ ] **Trending** — sección de posts más reaccionados de las últimas 24h
- [ ] **Búsqueda de posts** — por keyword, username, hashtag, tipo de post
- [ ] **Filtros de feed** — "Solo amigos", "Solo workouts", "Solo PR shares", "Solo mi gym"
- [ ] **Reportar contenido** — botón de reporte + flujo de moderación backend

#### Posts enriquecidos
- [ ] **Carrusel de imágenes** — múltiples imágenes en un solo post con swipe
- [ ] **Post de progreso** — side-by-side de foto antes/después
- [ ] **Video clips cortos** — límite de 60s, con preview de thumbnail automático
- [ ] **Share externo** — generar link de post público compartible en WhatsApp/Instagram
- [ ] **GIF de workout** — auto-generar GIF animado del resumen de workout para compartir
